...
async fn test_list_models() {
    ...
    let mock_response =
        json!({ "models": mock_models.iter().map(|m| json!({ "name": m })).collect::<Vec<_>>() });
    ...
    assert!(
        has_nonexistent.is_ok(),
        "has_model returned error: {:?}",
        has_nonexistent.err()
    );
    assert!(
        !has_nonexistent.unwrap(),
        "Expected nonexistent:latest to not exist"
    );
