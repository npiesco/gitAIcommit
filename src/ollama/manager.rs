use crate::ollama::{OllamaBinary, OllamaClient, OllamaClientTrait};
...
impl OllamaManager {
    ...
    pub async fn ensure_running(&mut self) -> Result<()> {
        ...
        if self.model.is_empty() {
            if let Ok(models) = self.client.list_models().await {
                if let Some(last_model) = models.last() {
                    println!(
                        "[INFO] No model specified, using last available model: {}",
                        last_model
                    );
        ...
        if self.model.is_empty() {
            return Err(GitAiError::Ollama(
                "No Ollama models found. Please install a model first with 'ollama pull <model>'"
                    .to_string(),
            )
            .into());
        }
    ...
    pub async fn generate_commit(&self, prompt: &str) -> Result<String> {
        self.client
            ...
            .await
            .map_err(|e| {
                GitAiError::Ollama(format!("Failed to generate commit message: {}", e)).into()
            })
    }
