use crate::git::{DiffInfo, FileChange, GitStatus};
...
impl GitInfo {
    ...
    pub fn is_empty(&self, after_staging: bool) -> bool {
        if after_staging {
            // After staging with --add-unstaged, staged changes and unresolved
            // conflicts must both keep the repository in a non-empty state.
            self.status.staged_files.is_empty() && self.status.unmerged_files.is_empty()
        } else {
            ...
            self.status.staged_files.is_empty()
                ...
                && self.status.unmerged_files.is_empty()
        }
    }
...
impl GitCollector {
    ...
    pub async fn collect_all(&self) -> Result<GitInfo> {
        ...
        let (status, diff_stat, branch_name, last_commit) =
            tokio::try_join!(status_task, diff_task, branch_task, last_commit_task)?;
    ...
    async fn get_diff_stat(&self) -> Result<DiffInfo> {
        ...
        if !staged_output.status.success() {
            ...
            return Err(
                GitAiError::Git(format!("Failed to get staged diff stats: {}", error)).into(),
            );
        }
        ...
        if !unstaged_output.status.success() {
            ...
            return Err(
                GitAiError::Git(format!("Failed to get unstaged diff stats: {}", error)).into(),
            );
        }
    ...
    async fn get_file_changes(&self) -> Result<Vec<FileChange>> {
        ...
        if !staged_output.status.success() {
            let error = String::from_utf8_lossy(&staged_output.stderr);
            return Err(
                GitAiError::Git(format!("Failed to get staged file changes: {}", error)).into(),
            );
        }
        ...
        if !unstaged_output.status.success() {
            let error = String::from_utf8_lossy(&unstaged_output.stderr);
            return Err(
                GitAiError::Git(format!("Failed to get unstaged file changes: {}", error)).into(),
            );
        }
        ...
        if !unstaged_changes.trim().is_empty() {
            ...
            for change in changes {
                if !all_changes
                    .iter()
                    .any(|c: &FileChange| c.file_path == change.file_path)
                {
    ...
    async fn get_untracked_files(&self) -> Result<Vec<PathBuf>> {
        ...
        if !output.status.success() {
            let error = String::from_utf8_lossy(&output.stderr);
            return Err(
                GitAiError::Git(format!("Failed to get untracked files: {}", error)).into(),
            );
        }
    ...
    pub async fn stage_all_unstaged(&self) -> Result<()> {
        ...
        if !output.status.success() {
            let error = String::from_utf8_lossy(&output.stderr);
            return Err(
                GitAiError::Git(format!("Failed to stage untracked files: {}", error)).into(),
            );
        }
